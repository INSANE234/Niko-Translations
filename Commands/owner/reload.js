    ...
    try {
      ...
      if(this.client.shard) {
        try {
          ...
          if(isCmd) 
            await msg.channel.sendOk(`🔄 Reloaded \`${cmdOrGrp.name}\` command, but failed to reload on other shards.`);
          else 
            await msg.channel.sendOk(
      ...
      if(isCmd) 
        await msg.channel.sendOk(`🔄 Reloaded \`${cmdOrGrp.name}\` command${this.client.shard ? ' on all shards' : ''}.`);
      else 
        await msg.channel.sendOk(
